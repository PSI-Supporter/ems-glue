...
class ReturnController extends Controller
{
    ...
    {
        $additionalWhere = [];
        $additionalWhere[] = ['RETSCN_SPLDOC', '=', $request->doc];

        if ($request->category) {
            $additionalWhere[] = ['RETSCN_CAT', '=', $request->category];
        }

        if ($request->line) {
            $additionalWhere[] = ['RETSCN_LINE', '=', $request->line];
        }
        ...
        $RSCountedPart = DB::table("RETSCN_TBL AS a")
            ->select(DB::raw("a.*,b.*, RTRIM(MITM_SPTNO) MITM_SPTNO, RTRIM(ISNULL(RETSCN_HOLD,'0'))  FLG_HOLD,ISNULL(SPL_RACKNO,'') SPL_RACKNO,item_value"))
            ...
            ->leftJoin('raw_material_labels', 'RETSCN_UNIQUEKEY', '=', 'code')
